...
run(){
	...
	timeout "$time" afl-fuzz -i input -o output -M fuzzer0 $dict -- ../build/aflppapp @@ &
	...
	while [ "$i" -lt "$threads" ]
       	do 
		timeout "$time" afl-fuzz -i input -o output -S fuzzer$i $dict -- ../build/aflppapp @@ &
	i=$(("$i" + 1))
	done
...
minimize() {
	...
	in_dir=$1
	out_dir=$2
	#command="/afl-cmin -i $in_dir -o $out_dir -- ./build/aflppapp @@"
	#echo $command
	time afl-cmin -i "$in_dir" -o "$out_dir" -- ./build/aflppapp @@
	# change it back since the normal run seems to be meddled by it
